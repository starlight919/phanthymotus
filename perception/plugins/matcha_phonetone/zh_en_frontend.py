...
class _FstNormalizer:
    ...
    def __call__(self, text: str) -> str:
        ...
        # The immutable r16 TN owns product-name normalization.
...
def normalize_text(text: str) -> str:
    """Normalize once before splitting text into G2P chunks."""
    _, normalizer = _resources()
    return _transliterate_non_cjk(normalizer(text))


def prepare_text(text: str, vocab_path: str | Path | None = None) -> FrontendResult:
    return prepare_normalized_text(normalize_text(text), vocab_path)


def prepare_normalized_text(
    normalized: str, vocab_path: str | Path | None = None,
) -> FrontendResult:
    """Encode already-normalized text without running TN or transliteration."""
    overlay, _ = _resources()
    tokens = []
